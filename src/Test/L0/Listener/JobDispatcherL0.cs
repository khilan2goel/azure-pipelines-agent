    ...
    public sealed class JobDispatcherL0
    {
        ...
        public async void DispatchesJobRequest()
        {
            ...
            using (var hc = new TestHostContext(this))
            {
                ...
                Assert.False(jobDispatcher.OneTimeJobCompleted.Task.IsCompleted, "JobDispatcher should not set task complete token for regular agent.");
            }
        }
        ...
        public async void DispatchesOneTimeJobRequest()
        {
            ...
            using (var hc = new TestHostContext(this))
            {
                ...
                _configurationStore.Setup(x => x.GetSettings()).Returns(new AgentSettings() { PoolId = 1, OneTime = true });
                jobDispatcher.Initialize(hc);
                ...
                Assert.True(jobDispatcher.OneTimeJobCompleted.Task.IsCompleted, "JobDispatcher should set task complete token for one time agent.");
                Assert.True(jobDispatcher.OneTimeJobCompleted.Task.Result, "JobDispatcher should set task complete token to 'TRUE' for one time agent.");
            }
        }
    }
}
